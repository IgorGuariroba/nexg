...
class UsuarioController {
    ...
    //GET/:id
     ...
     store(req, res, next){
         ...
         if(!nome || !email || !password) return res.status(422).json({errors: "Preencha todos os campos de cadastros"});
     ...
     update(req, res, next){
         ...
         Usuario.findById(req.payload.id).then((usuario) => {
             if(!usuario) return res.status(401).json({ errors: "Usuario não registrado"});
             if(typeof nomme !== "undefined") usuario.nome = nome;
     ...
     createRecovery(req, res, next){
         ...
         Usuario.findOne({ email }).then((usuario) => {
             if(!usuario) return res.render("recovery", {errors: "Não existe usuário com este email", success: null});
             const recoveryData = usuario.criarTokenRecuperacaoSenha();
             return usuario.save().then(() => {
                return res.render("recovery", {errors: null, success: true});
             }).catch(next);
         }).catch(next);
     }
     ...
     showCompleteRecovery(req, res, next){
        ...
        Usuario.findOne({"recovery.token": req.query.token}).then(usuario => {
            ...
            if(new Date(usuario.recovery.date) < new Date() ) return res.render("recovery", {error: "Token expirado. Tente novamente.", success: nukl});
     ...
     completeRecovery(req, res, next){
         ...
         if(!token || !password) return res.render("recovery/store", {error: "Preencha novamnete com sua nova senha"});
         Usuario.findOne({ "recovery.token": token}).then(usuario => {
             ...
             return usuario.save().then(() => {
                 return res.render("recovery/store", {
                     error: null,
                     success: "Senha alterada com sucesso. Tente novamnete fazer o login",
                     token: null
                 })
             }).catch(next);
         });
